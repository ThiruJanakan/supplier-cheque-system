...
export default function Cheques() {
  ...
  const allocTotal = editing ? editing.allocations.reduce((s, a) => s + (+a.allocated_amount || 0), 0) : 0;
        ...
        <Modal title={editing.id ? `Edit cheque ${editing.cheque_number}` : 'Register cheque'} onClose={() => setEditing(null)}
          ...
          <div className="grid cols-2">
            ...
            <Field label="Supplier *">
              <select value={editing.supplier_id} onChange={e => setEditing({ ...editing, supplier_id: +e.target.value, allocations: [] })}>
            ...
            return (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <select style={{ flex: 2 }} value={a.purchase_id || ''} onChange={e => {
                  const next = [...editing.allocations]; next[i] = { ...a, purchase_id: +e.target.value };
          ...
          {editing.allocations.length > 0 && (
            <div className="mono" style={{ marginTop: 10, fontSize: 12.5, color: allocTotal > +editing.amount ? 'var(--claret)' : 'var(--muted)' }}>
              Allocated {allocTotal.toFixed(2)} of {(+editing.amount || 0).toFixed(2)}
            </div>
          )}
        </Modal>
      )}
