...
export default function Settings() {
  ...
  const backupNow = async () => {
    try { await api.post('/system/backups'); setNotice('Backup created.'); load(); }
    catch (e) { setError(e.message); }
  };
      ...
      <div className="grid cols-2">
        <div className="card card-pad">
          ...
          <Field label="Overdue warnings">
            <select value={settings.overdue_alerts} onChange={e => setSettings({ ...settings, overdue_alerts: e.target.value })}>
              <option value="1">Send daily warning for overdue cheques</option>
        ...
        <div>
          ...
          <div className="card card-pad">
            ...
            <p className="muted" style={{ marginTop: 0 }}>Backups run automatically every night. The newest {backups.length} copies are kept in <span className="mono">backend/data/backups</span>.</p>
            <button className="btn ghost" onClick={backupNow}>Back up now</button>
            <table style={{ marginTop: 12 }}>
              <thead><tr><th>File</th><th className="num">Size</th></tr></thead>
              <tbody>
                {backups.length === 0 && <tr><td colSpan={2} className="empty">No backups yet.</td></tr>}
